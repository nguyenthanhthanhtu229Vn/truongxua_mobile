...
const POST = [
  {
    ...
    content: 'Supervision as a Personnel Development Device Nam eget dui. Etiam rhoncus. Maecenas tempus, tellus eget condimentum rhoncus, sem quam semper libero.',
    ...
    <View style={{marginTop: 150}}>
      <FlatList 
        ...
        renderItem={({item, index}) => {
          return (
            <View style={{height: 450,backgroundColor: COLORS.white2,shadowOpacity: 0.4,marginBottom: 16}}>
              <View style={{flexDirection: 'column', marginLeft: 10,marginTop: 20,}}>

                <View style ={{flexDirection: 'row'}}>
                <Image source={item.avatar} style ={{height: 60, width: 60, borderRadius: SIZES.largeTitle}}/>
                <Text style={{color: COLORS.blue, marginLeft: 4, ...FONTS.h3,fontWeight: '500'}}>{item.name}</Text>
                </View>
          
                <View style ={{flexDirection: 'row',marginTop: -34,marginLeft: 64}}>
                  <Image source={icons.globe} style={{height: 20,width: 20,}} />
                  <Text style ={{marginLeft: 8}}>{item.date}</Text>
                </View>
                <Text style={{marginTop: 20, ...FONTS.h3,color: COLORS.black, marginBottom: 10}}>{item.content}</Text>
                <Image source={item.imagesPost} style={{width: 364}} />
